...
const CHAIN_NAMES_LIST = [
  "ethereum", "base", "arbitrum", "optimism", "polygon", "gnosis", "xdai",
  "linea", "scroll", "zksync", "mantle", "monad", "abstract", "zora", "unichain", "bsc", "binance",
];

const CHAIN_NORMALIZE: Record<string, string> = {
  ethereum: "ethereum", base: "base", arbitrum: "arbitrum", optimism: "optimism",
  polygon: "polygon", gnosis: "gnosis", xdai: "xdai", linea: "linea",
  scroll: "scroll", zksync: "zksync-era", mantle: "mantle", monad: "monad",
  abstract: "abstract", zora: "zora", unichain: "unichain",
  bsc: "binance-smart-chain", binance: "binance-smart-chain",
};

const CHAIN_PAT = CHAIN_NAMES_LIST.join("|");
const ASSET_CHAIN_RE = new RegExp(`\\b(\\d+(?:[.,]\\d+)?(?:e-?\\d+)?)\\s+([A-Za-z]{2,10})\\s+on\\s+(${CHAIN_PAT})\\b`, "gi");
...
const KNOWN_SYMBOLS = new Set([
  "ETH", "WETH", "USDC", "USDT", "DAI", "WBTC", "GNO", "ARB", "OP", "MATIC", "POL",
  "MNT", "PENDLE", "ZORA", "DEGEN", "RNBW", "SCR", "MON", "CLAWD", "CLAWNCH", "ABS",
  "VIBE", "GIV", "HNY", "RAID", "XDAI", "STAKE", "LOOKS", "ALEX", "FOX", "LPT",
  "BNKRW", "LVUSDC", "SALT", "WRLD", "SOCIAL", "JOON", "DREAMBOY", "BNB", "wstETH", "weETH",
]);
...
export default function ChatMessageRenderer({ content, portfolio }: ChatMessageRendererProps) {
  ...
  return (
    <p
      className="whitespace-pre-wrap leading-relaxed m-0 font-[family-name:var(--font-literata)]"
